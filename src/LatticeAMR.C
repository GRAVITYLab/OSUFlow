...
// default = 0 (can omit if single process sequential program)
//
LatticeAMR::LatticeAMR(char **files, int tlen, int ntpart, char *vx, 
		       char *vy, char *vz, DataMode dm, float scale, int nid, 
		       int myid, int mpi) {
  ...
  if (mpi) {
    t0 = MPI_Wtime();
    tamr->LoadMetaData(files, min, max, tlen, block_dims, dm, scale); 
  }
  else
    tamr->SerialLoadMetaData(files, min, max, tlen, block_dims, dm, scale); 
#else
  tamr->SerialLoadMetaData(files, min, max, tlen, block_dims, dm); 
#endif
#ifdef _MPI
  if (mpi)
...
// dm: data mode
//
void LatticeAMR::LoadData(int t, DataMode dm) {
  ...
  amr->LoadHDF5Data(start_block, end_block, vx, vy, vz, dm, mpi);
  for (i = 0; i < amr->GetNumBlocks(); i++) {
...
//
void LatticeAMR::PostPoint(int block, float *p, int recirc) {

  // neighbor number of neighbor block
  int neighbor = GetNeighbor(block, p[0], p[1], p[2], p[3]);

  // deprecated, replace with code below
...
// //
// int LatticeAMR::OldExchangeNeighbors(float ***seeds, int *alloc_seeds,
...
// // returns total number of points received by this process
// //
// int LatticeAMR::SyncExchangeNeighbors(int *nproc, float ***pts, int ***cts,
// 				    int64_t ***pids) {
