...
int dims[3];           // dimensions of each block
float *mins, *maxs;    // two global min/max corners 
float *centers;        // centers of each block 
float *lengths;        // physical lengths of each block
...
float *element_xsize;  // x length of a cell in each block 
float *element_ysize;  // y length of a cell in each block 
float *element_zsize;  // z length of a cell in each block 
...
void LoadAMR(char* fname, float* min, float *max)
{
  ...
  element_xsize = new float[nb]; // the physical length in x of a cell in each block  
  element_ysize = new float[nb]; // the physical length in y of a cell in each block 
  element_zsize = new float[nb]; // the physical length in z of a cell in each block 
  ...
  level_xsize = new float[num_levels]; //  the physical lengths of a block in each level 
...
main(int argc, char* argv[]) 
{
  ...
  // len[] are the physical lengths of the domain 
  latticeAMR = new LatticeAMR(len[0], len[1], len[2], 1.0, num_levels); 
  ...
  for (int i=0; i<num_levels; i++)  {
    ...
    // create a level in the lattice data structure 
  ...
  for (int i=0; i<nb; i++) {
    // register the block to the lattice data structure 
    latticeAMR->CheckIn(its_level[i], centers[i*3], centers[i*3+1], centers[i*3+2], 0.0, 
			vectors[i]); 
  }

  latticeAMR->MergeBlocks();   // this is what's new in gldrawFlash4 - we try to merge adjacent blocks that have the 
                               // same resolutions together into larger blocks to minimize overhead 
