...
static CDynamicColorTubeRenderer TubeRenderer;
static CDynamicColorLineRenderer LineRenderer;
static bool tube = 0; // tubes or lines
static float col[3]; // rendering color
static bool random_color; // whether color is specified or colors chosen randomly
//----------------------------------------------------------------------------
//
...
//----------------------------------------------------------------------------
//
// initializes drawing, float vector format
//
// p: points, float format
// np: number of points in each trace
// nt: number of traces
...
void key(unsigned char key, int x, int y) {
  ...
  case '0': 
    if (tube)
      TubeRenderer._GenerateRandomColor();
    else
      LineRenderer._GenerateRandomColor();
    glutPostRedisplay();
    break;
  case '1': 
    if (tube)
      TubeRenderer._GenerateRandomFlags();
    else
      LineRenderer._GenerateRandomFlags();
    glutPostRedisplay();
    break;
  case 'h':
    int halo;
  ...
  if (tube) {
    ...
    if (random_color)
      TubeRenderer._SetInteger(CTubeRenderer::COLOR_SCHEME, 
			       CTubeRenderer::CColorScheme::COLOR_ON_THE_FLY);
    else
      TubeRenderer._SetInteger(CTubeRenderer::COLOR_SCHEME, 
    ...
    if (random_color)
      LineRenderer._SetInteger(CLineRenderer::COLOR_SCHEME, 
			       CLineRenderer::CColorScheme::COLOR_ON_THE_FLY);
    else
      LineRenderer._SetInteger(CLineRenderer::COLOR_SCHEME, 
...
void trace_lists() {
  ...
  if (tube) {
    ...
    TubeRenderer._GenerateRandomColor();
    TubeRenderer._SetAllFlags();
  }
  else {
    LineRenderer._Update();
    LineRenderer._GenerateRandomColor();
    LineRenderer._SetAllFlags();
  }
