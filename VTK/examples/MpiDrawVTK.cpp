...
OSUFlowVTK **osuflow; // one flow object for each block
...
const float initialStepSize = 0.1;
...
//const TRACE_DIR integrationDir = BACKWARD_AND_FORWARD; // FORWARD_DIR
const TRACE_DIR integrationDir = FORWARD_DIR;
...
void Init() {
  ...
  assert((osuflow = (OSUFlowVTK**)malloc(nblocks * sizeof(OSUFlowVTK))) != NULL);
  for (i = 0; i < nblocks; i++)
    osuflow[i] = new OSUFlowVTK;
...
// 			&pt, &npt, &tot_ntrace, nblocks, 0);
  ...
  parflow = new ParFlow(blocks, (OSUFlow **)osuflow, sl_list,
			&pt, &npt, &tot_ntrace, nblocks, 0);
  // end TP
  ...
  parflow->SetIntegrationDir(integrationDir);
