...
main(int argc, char* argv[])
{
    ...
    data = vtkMultiBlockDataSet::SafeDownCast(reader->GetOutput());
    vtkDataSet *data0 = vtkDataSet::SafeDownCast(data->GetBlock(0));

	//int *dim = grid->GetDimensions();
	double *bounds = data0->GetBounds();
	printf("bounds: %lf %lf %lf %lf %lf %lf\n", bounds[0], bounds[1], bounds[2], bounds[3], bounds[4], bounds[5]);

	osuflow = new OSUFlow;
	CVectorField *field = new MultiBlockVectorFieldVTK( data );
